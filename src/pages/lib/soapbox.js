...
function redirect_username(username, options) {
    return `${utils.protocolHost(options.soapbox.instance)}/@${username}`
}
...
function redirect_post_comment(post_comment, options) {
    return new Promise(resolve => {
        const req = new XMLHttpRequest()
        req.open("GET", `${options.soapbox.instance}/api/v2/search?q=${encodeURIComponent(post_comment)}&resolve=true&limit=1`, true);
        ...
        req.onload = () => {
            ...
            resolve(`${utils.protocolHost(options.soapbox.instance)}/@${username}/posts/${post_id}`)
        }
        req.send()
    })
}
